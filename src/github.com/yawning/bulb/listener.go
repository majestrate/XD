...
func (c *Conn) NewListener(config *NewOnionConfig, vports ...uint16) (net.Listener, *OnionInfo, error) {
	...
	if !ok {
		tcpListener.Close()
		return nil, nil, newProtocolError("failed to extract local port")
	}

	if len(vports) < 1 {
		return nil, nil, newProtocolError("no virual ports specified")
	}
...
func (c *Conn) Listener(port uint16, key crypto.PrivateKey) (net.Listener, error) {
	...
	l, _, err := c.NewListener(cfg, port)
	return l, err
}
